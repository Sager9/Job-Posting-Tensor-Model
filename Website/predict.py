...
def makePredictionTF(url):
    ...
    prediction_p = model.predict(tokened_data)

    #Prints fraud probabliity to terminal
    print(prediction_p)

    #Fraud = 1 and legitimate = 0
    #adjusted mid point so only verry fraudlent jobs would be marked as such
    prediction =np.where(prediction_p > .90, 1,0)
